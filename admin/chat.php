...
// Temukan ID pengguna admin. Asumsi peran 'Admin' memiliki id_role = 1
$admin_id = null;
...
if ($result_admin_id && mysqli_num_rows($result_admin_id) > 0) {
    ...
    // Tangani kasus di mana pengguna admin tidak ditemukan
    die("Admin user not found. Please ensure an admin user exists in the database.");
}

// Periksa apakah ada chat room yang sudah ada antara pengguna saat ini dan admin
$id_chat_room = null;
...
if ($result_chatroom && mysqli_num_rows($result_chatroom) > 0) {
    ...
    // Jika tidak ada chat room, buat yang baru
    ...
    if (mysqli_query($koneksi, $insert_chatroom_query)) {
        $id_chat_room = mysqli_insert_id($koneksi); // Dapatkan ID dari chat room yang baru dibuat
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['message_content'])) {
    ...
    // Redirect untuk mencegah pengiriman form ulang saat refresh
