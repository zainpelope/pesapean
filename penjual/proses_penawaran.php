...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($harga_tawaran <= $harga_tertinggi_saat_ini) {
        ...
        header("Location: ../penjual/detail.php?id=" . $id_sapi . "&status=failed");
        exit;
    }
    ...
    if ($insert_penawaran) {
        ...
        if ($update_lelang) {
            header("Location: ../penjual/detail.php?id=" . $id_sapi . "&status=success");
        } else {
            ...
            header("Location: ../penjual/detail.php?id=" . $id_sapi . "&status=failed_update_lelang");
        }
    } else {
        header("Location: ../penjual/detail.php?id=" . $id_sapi . "&status=failed_insert_penawaran");
    }
} else {
