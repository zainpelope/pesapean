...
$query = mysqli_query($koneksi, "
    SELECT
        ...
        l.status,
        l.batas_waktu,
        ds.id_user_penjual, -- *** Corrected: Use id_user_penjual from data_sapi table ***
...
if (mysqli_num_rows($query) == 0) {
    echo "Data sapi tidak ditemukan atau tidak memiliki lelang terkait.";
    exit;
}
...
$loggedInUserRole = isset($_SESSION['role']) ? $_SESSION['role'] : null;

// Tentukan apakah pengguna adalah pemilik sapi ini atau admin
$is_owner_or_admin = false;
if ($loggedInUserId) {
    // Corrected: Use id_user_penjual for comparison
    if (isset($sapi['id_user_penjual']) && $sapi['id_user_penjual'] == $loggedInUserId) {
        $is_owner_or_admin = true; // User adalah pemilik sapi yang dilelang
    } elseif ($loggedInUserRole == 'admin') {
        $is_owner_or_admin = true; // User adalah admin
    }
}
    ...
    <div class="container mt-5">
        <div class="detail-box shadow">
            ...
            <?php if ($lelang_aktif) : ?>
                ...
                <?php if ($loggedInUserId) : // Tampilkan form penawaran jika user sudah login (jika ada) 
                ?>
                <?php else : ?>
                ...
                <?php endif; ?>
            <?php else : // Lelang tidak aktif (sudah berakhir atau status lain) 
            ?>
                <div class="alert alert-info text-center mt-4">
                    ...
                    <?php if ($sapi['status'] == 'Lewat' && !empty($sapi['nama_penawar_tertinggi'])): ?>
                        ...
                        <?php if ($loggedInUserId && $sapi['id_penawar_tertinggi'] == $loggedInUserId): ?>
                            <hr>
                            <p class="text-center">
                                <span class="badge bg-success mt-1">Selamat, Anda memenangkan lelang ini!</span><br>
                                Silakan klik tombol di bawah untuk proses pembayaran.
                            </p>
                            <div class="d-grid gap-2">
                                <a href="../pembeli/pembayaran.php?id_lelang=<?= htmlspecialchars($sapi['id_lelang']); ?>&id_pemenang=<?= htmlspecialchars($sapi['id_penawar_tertinggi']); ?>" class="btn btn-primary btn-lg">Lanjutkan ke Pembayaran</a>
                            </div>
                        <?php elseif ($is_owner_or_admin && $sapi['status'] != 'Selesai'): ?>
                            <hr>

                            <div class="d-grid gap-2">
                                <a href="../penjual/konfirmasi_pembayaran.php?id_lelang=<?= htmlspecialchars($sapi['id_lelang']); ?>&id_pemenang=<?= htmlspecialchars($sapi['id_penawar_tertinggi']); ?>" class="btn btn-success btn-lg">Konfirmasi Pembayaran</a>

                            </div>
                        <?php endif; ?>
                    <?php elseif ($sapi['status'] == 'Selesai'): ?>
                        <div class="alert alert-success text-center mt-3">
                            Lelang ini telah **Selesai** dan pembayaran telah dikonfirmasi.
                            <br>Pemenang: <strong><?= htmlspecialchars($sapi['nama_penawar_tertinggi'] ?? 'Tidak Diketahui'); ?></strong>
                        </div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            // Corrected: Use 'harga_tawaran' and 'waktu_tawaran' from your 'penawaran' table
            $queryBidHistory = mysqli_query($koneksi, "
                SELECT
                    p.harga_tawaran,   -- *** Corrected column name ***
                    p.waktu_tawaran,   -- *** Corrected column name ***
                    u.username,
                    u.id_user
                FROM Penawaran p
            ...
            <?php if (mysqli_num_rows($queryBidHistory) > 0): ?>
                <hr>
                <h5 class="mt-4 text-center">Riwayat Penawaran Lelang Ini</h5>
                <ul class="list-group">
                    <?php while ($bid = mysqli_fetch_assoc($queryBidHistory)): ?>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <?php endif; ?>
                                <br><small class="text-muted"><?= date("d M Y H:i:s", strtotime($bid['waktu_tawaran'])); ?></small>
                            </div>
            ...
            <div class="text-center mt-4">
                <a href="../penjual/lelang.php" class="btn btn-secondary">Kembali ke Daftar Lelang</a>
            </div>
        </div>
    </div>
            ...
            if (status) {
                ...
                switch (status) {
                    case 'success':
                        ...
                        text = 'Tindakan Anda telah berhasil diproses.';
                        break;
                    case 'failed':
                        icon = 'error';
                        title = 'Operasi Gagal!';
                    ...
                    case 'not_logged_in':
                        ...
                        text = 'Anda harus login untuk melakukan tindakan ini.';
                        break;
                    ...
                    case 'invalid_data':
                        ...
                        text = 'Data tidak lengkap atau tidak valid.';
                        break;
                    case 'already_bid':
                        icon = 'info';
